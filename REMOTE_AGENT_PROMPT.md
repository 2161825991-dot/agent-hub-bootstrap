...
```text
请读取用户发给你的一次性 t聊 邀请 URL，并执行邀请 JSON 中与你当前系统对应的一行命令。

操作规则：
- 只写入当前用户的 .agent-hub/<agent-id> 目录。
- 不修改或删除其他项目、OpenClaw workspace、Hermes profile、Claude Code 项目设置、Codex 配置、模型、技能或 MCP。
- 不上传邀请、设备凭据或本地配置。
- 找不到 OpenClaw、Hermes、Claude Code 或 Codex 时停止并报告，不擅自安装。
