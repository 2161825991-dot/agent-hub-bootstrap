...
```text
invite_url: "One-time URL generated by Agent Hub App"
```

When an invite URL is available, read `START_HERE_FOR_INVITE.md`. Do not ask the user for a long-lived Token.

The values below are only for advanced legacy setup when no invite is available:
