...
这是 Agent Hub 的公开接入程序仓库。它只保存通用脚本、连接器和说明，不保存邀请、设备凭据、管理 Token、用户配置或局域网地址。

## 普通用户只做三步

1. 在 Agent Hub App 点击“添加 Agent”，选择 OpenClaw 或 Hermes。
...
- 不修改或删除 OpenClaw workspace、Hermes profile、模型、技能或其他 MCP。
- 不修改 Mac 上的 Agent Hub 服务端。
- 不 reset、clean、覆盖或删除用户项目。
- 不把邀请 URL、Token 或生成的配置提交到 GitHub。
- 找不到 OpenClaw/Hermes 时停止并报告，不代替用户安装 Agent。
